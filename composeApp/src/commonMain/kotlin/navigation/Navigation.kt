...
fun Navigation(navController: NavHostController = rememberNavController()) {
    ...
    val navigateTo =
        remember {
            { destination: NavDestination -> navController.navigate(route = destination) }
        }
    // Define how to pass keys around for loading items from db:
    // avoid passing the objects themselves
    val typeMap =
        mapOf(
