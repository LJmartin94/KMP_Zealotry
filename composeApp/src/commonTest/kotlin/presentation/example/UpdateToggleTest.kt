...
class UpdateToggleTest {
    ...
    @Test
    fun `when id is null sets error and does not call repository`() = runTest {
    ...
    @Test
    fun `when id is set and update succeeds loading is cleared and no error`() = runTest {
        val repo = mock<ExampleRepository>()
    ...
    @Test
    fun `when id is set and update fails error state is set`() = runTest {
        val repo = mock<ExampleRepository>()
